...
const _type_rate RatesSolver::rate(Lattice &source_lattice, Lattice &target_lattice,
  ...
  const _type_lattice_id source_lat_id = source_lattice.id;
  const _type_lattice_id target_lat_id = target_lattice.id;
  ...
  const double active_energy =
      e0(ghost_atom) + deltaE(source_lat_id, target_lat_id, source_lattice, target_lattice, ghost_atom) / 2;
