...
//                     const LatticeTypes::lat_type types[], unsigned long count[], const int len);
...
//             << "| MnMn\t|" << m_counter.get(LatticeTypes::MnMn) << std::endl
//             << std::endl;
// }

// //开始
 int main() {
//   r::initSeed(0x146221); // initialize random number seed // todo generate seed here //初始化随机数种子

//   // set configuration
//   env::global_env = env::environment{6E12, // attempt_freq 6*10^12 // todo too big
//                                      773,  // temperature, K
//                                      3.53, // ef110
...
//   counter m_counter;
//   //这是一个Lambda表达式，它定义了一个匿名函数，用于对每个格点执行操作。Lambda表达式使用[&]捕获了外部作用域的所有变量，以便在Lambda函数内部使用。
//   //这段代码的作用似乎是遍历lattice_list中的每个格点，并为每个格点随机设置类型，然后对类型进行计数。Lambda函数提供了一种方式来定义如何处理每个格点的逻辑。
//   sim_box->lattice_list->forAllLattices([&](const _type_lattice_coord x, const _type_lattice_coord y,
//                                             const _type_lattice_coord z, Lattice &lattice) {
//     // set lattice types randomly.
//     const LatticeTypes::lat_type src_types[] = {LatticeTypes::Fe, LatticeTypes::Cu, LatticeTypes::Ni, LatticeTypes::Mn};
