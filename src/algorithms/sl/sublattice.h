...
public:
   ...
   */
  template <typename E> _type_rate calcRatesWrapper(ModelAdapter<E> *p_model, const type_sector_id sector_id, bool *sector_first, int sect, int is_last_step);
...
private:
   ...
   */
  template <class PKs, class Ins> void syncSimRegions(Ins &pk_inst, std::array<std::vector<ChangeLattice>, 7>& exchange_ghost, std::array<std::unordered_set<_type_lattice_id>, 8>& exchange_surface_x,
  ...
  template <class PKs, class Ins> void syncSimGohstRegionsCombine(Ins &pk_inst, std::array<std::vector<ChangeLattice>, 7>& exchange_ghost, 
