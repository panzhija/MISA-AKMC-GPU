...
public:
  ...
  // std::unordered_set<unsigned long> vac_hash;
  std::unordered_set<_type_lattice_id> cu_hash;
  std::unordered_set<_type_lattice_id> mn_hash;
  std::unordered_set<_type_lattice_id> ni_hash;

  // 添加检查机制，确保 latti_id 只存在于一个哈希表中
  void isUniqueLattiId(const _type_lattice_id& latti_id) {
    int count = 0;
    if (vac_hash.find(latti_id) != vac_hash.end()) count++;
    if (cu_hash.find(latti_id) != cu_hash.end()) count++;
    if (mn_hash.find(latti_id) != mn_hash.end()) count++;
    if (ni_hash.find(latti_id) != ni_hash.end()) count++;
